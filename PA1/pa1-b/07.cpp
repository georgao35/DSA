...
int main()
{
	...
	while (m--)
	{
		...
		checkLeft(who, lParent, head, count, value);	//检查后，保证0<=head<=lParent->size()-1：如果cur_rank是0的时候，head将一直是-1；
		...
		if (count <= 2)
		{
			...
			while (p != END)
			{
				for (int i = 0; i < p->size(); i++)
				{
					putchar(p->balls[i]); //p的元素并没有真删除，而是因为size改变了
					empty = false;
				}
				p = p->next;
			}
		...
		char left = lParent->balls[head];//越界访问✅
